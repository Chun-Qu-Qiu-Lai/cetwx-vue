<template>
	<view style="padding-top: 10%;">
		...
		<!-- 
<free-audio startPic='../../static/play.png' endPic='../../static/stop.png' :audioId='audio1' :url='music'></free-audio> -->
		...
		<uni-popup ref="popupIsSelected" backgroundColor="">
			<view class="trangle-flex"
				style="background-color: #e9eae5;border-radius:15rpx;display: grid;grid-template-columns: repeat(5,1fr);">
				<view class="trangle-flex-circular" v-for="(item,index) in list" :key="index"
					:style="[item.selected?selde:noselde]"
		...
		<view class="part">
			...
			<view class="part-subtitle">
				<text decode='true'>&nbsp;&nbsp;</text>Directions: In this section, you will hear threenews reports. At
...
<script>
	/* 	import freeAudio from '../../uni_modules/chengpeng-audio/free-audio.vue' */
	import
	WzsAudio
	...
	export default {
		...
		data() {
			return {
				...
				section: null,
				cetFourOrSix: null,
				music: 'https://www.shanzs.top/cetfile/cetFourListen/6/6-0-0.mp3',
		...
		onLoad(options) {
			...
			/* 	if (this.cetFourOrSix == 4) {
					this.music = requestUrl.prod.BASE_URL + '/cetFourListen' + '/' + this.yearId + '/' + this.yearId +
						'-' +
